...
# Функция для создания перехода с текстом
def create_transition(text, duration=2):
    # Создаем синий фон
    bg = Image.new('RGB', TARGET_RESOLUTION, color=(0, 0, 255))
    draw = ImageDraw.Draw(bg)
    # Выбираем шрифт и размер
    font = ImageFont.load_default()
    text_size = draw.textsize(text, font=font)
    # Позиционируем текст по центру
    position = ((TARGET_RESOLUTION[0] - text_size[0]) // 2, (TARGET_RESOLUTION[1] - text_size[1]) // 2)
    # Рисуем текст
    draw.text(position, text, fill=(255, 255, 255), font=font)
    # Конвертируем в видеофайл
    img = np.array(bg)
    transition_clip = mpy.ImageClip(img).set_duration(duration)
    return transition_clip

def create_text_image(text):
    img = Image.new('RGB', (TARGET_RESOLUTION[0], TARGET_RESOLUTION[1]), color=BLUE_COLOR)
...
# Создаем переходы
transitions = []
num_chunks = len(VideoChunk.chunks)
for idx, chunk in enumerate(VideoChunk.chunks):
    number = num_chunks - idx
    text = f"number {number}"
    transition = create_transition(text)
    transitions.append(transition)
    # Добавляем переход перед чанком
...
final_video = mpy.concatenate_videoclips(final_clips)

# Обработка аудио
audio_clips = [mpy.VideoFileClip(chunk.source_path).subclip(chunk.start, chunk.end).audio for chunk in VideoChunk.chunks]
